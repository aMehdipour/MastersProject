...
for method = methods
    ...
    for j =  1:length(gyroNoiseDensity)
        ...
        dataSave.bankCmd(:, j, method + 1) = rad2deg(cmd_a(1,1:length(aoa)-1));
        dataSave.aoaCmd(:, j, method + 1) = rad2deg(cmd_a(2,1:length(aoa)-1));
        dataSave.ssCmd(:, j, method + 1) = rad2deg(cmd_a(3,1:length(aoa)-1));
...
plot(dataSave.time(1:end-1,j,i),dataSave.bankCmd(:,1,1), 'color', colors(count,:), 'linew', 2)
Legend{count} = 'Cmd';
...
plot(dataSave.time(1:end-1,j,i), dataSave.aoaCmd(:,1,1), 'color', colors(count,:), 'linew', 2)
...
plot(dataSave.time(1:end-1,1,1), dataSave.ssCmd(:,1,1), 'color', colors(count,:), 'linew', 2)
Legend{count} = 'SS Cmd';
...
tiledlayout(2,1)
nexttile
plot(dataSave.time(:,1,1), dataSave.V(:,1,1), 'LineWidth', 2)
xlabel('Time (s)')
ylabel('Velocity (m/s)')
