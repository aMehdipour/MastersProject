function [v_aero,B,bias,f_aero,Ffit] = ControllerAero(V,rho,aoa,ss,bank,fd,CFD)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Lm_A_pre = M_const*interp2(avec, svec, CFD.A.CmL,ss,aoa); %aero moments computed in aerodynamic frame
Mm_A_pre = M_const*interp2(avec, svec, CFD.A.CmM,ss,aoa);
Nm_A_pre = M_const*interp2(avec, svec, CFD.A.CmN,ss,aoa);
...
for i = 1:5
    CL_A(i) = interp2(avec, svec, CFD.A.Cl,ss,angvec(i)); %query points ordering is flipped
    CS_A(i) = interp2(avec, svec, CFD.A.Cs,angvec(i),aoa);
end
