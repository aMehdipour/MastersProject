...
constants.C1                    = 5.21e-3;
constants.C0                    = 8.71e-5;
...
state.bankSign               = -sign(state.crossrange);
...
tic
%% Simulation loop
while state.altitudeUnnormalized > 0
    ...
    if t == 0 || (t - lastGuidanceCall >= parameters.guidanceRate)
        ...
        if ~flagInsideDeadband
            if state.crossrange * state.bankSign > 0
                state.bankSign = -state.bankSign;
                flagInsideDeadband = true;
            end
        else
            if abs(state.crossrange) > calculateDeadbandWidth(state.velocity, constants)
                flagInsideDeadband = false;
            end
        end            
    ...
    [state, derivatives] = updateState(state, commandedBankAngle, constants, parameters);
